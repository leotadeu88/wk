...
var
  ...
  fdq: TFDQuery;
begin
  InputQuery('Carregar pedido','Informe o número do pedido à ser cancelado',nPedido);
  if nPedido<>'' then
  begin
    ...
    FreeAndNil(fdq);
    StatusBar1.Panels[0].Text := 'Total do pedido: R$ 0,00';
  end;
end;
...
begin
  Total_Pedido := 0;
  InputQuery('Carregar pedido','Informe o número do pedido à ser carregado',nPedido);
  if nPedido<>'' then
  begin
    fdq := TFDQuery.Create(Application);
    with fdq do
    begin
      ...
      begin
        ...
        MessageBox(Handle, 'Pedido não foi localizado', 'Informação', MB_OK +
          MB_ICONINFORMATION + MB_DEFBUTTON2);
    end;

    FreeAndNil(fdq);
    StatusBar1.Panels[0].Text := 'Total do pedido: R$ '+FormatFloat('#.00',Total_Pedido);
  end;
end;

function TFrmPrincipal.BuscaNomeCliente(vCodigo: Integer): string;
var
  vSql : String ;
begin
...
function TFrmPrincipal.BuscaValorTotal: Currency;
var
  vTotal : Currency ;
begin
  vTotal := 0 ;
...
begin
  ...
  begin
    ...
    MessageBox(Handle, 'Informe o cliente', 'Atenção', MB_OK + MB_ICONWARNING + MB_DEFBUTTON2);

end;

procedure TFrmPrincipal.CarregarComboCliente;
var
  fdq: TFDQuery;
  cliente: TCliente;
begin
...
procedure TFrmPrincipal.AtualizaTotalPedido;
var
  TotalPedido:Double;
begin
  TotalPedido := 0;
  ...
  while not cdsProdutos.Eof do
  begin
    TotalPedido := TotalPedido + cdsProdutosvlr_total.AsFloat;
    cdsProdutos.Next;
  end;
  StatusBar1.Panels[0].Text := 'Total do pedido: R$ ' + FormatFloat('#.00', TotalPedido);
end;
  ...
  begin
    ...
    begin
      ...
      if Key = VK_DELETE then
        if MessageBox(Handle, 'Deseja apagar o produto do pedido ?','Confirmação', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) = IDYES
        then
         begin
           cdsProdutos.Delete;
           AtualizaTotalPedido;
...
procedure TFrmPrincipal.edtCodigoClienteKeyPress(Sender: TObject; var Key: Char);
begin
